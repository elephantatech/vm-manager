...
class VMControl:
    ...
    async def stop_vm(self, vmx_path: str, mode: str = "soft") -> bool:
        ...
        if returncode != 0:
            logger.warning(
                {
                    "event": "vmrun_soft_stop_failed",
                    "vmx": vmx_path,
                    "error": stderr,
                    "message": "Retrying with hard stop...",
                }
            )
            # Fallback to hard power off if soft fails
            returncode, stdout, stderr = await self._run_vmrun(
                ["-T", "ws", "stop", vmx_path, "hard"]
            )
            if returncode != 0:
                logger.error(
                    {
                        "event": "vmrun_hard_stop_failed",
                        "vmx": vmx_path,
                        "error": stderr,
                    }
                )
    ...
    async def restart_vm(self, vmx_path: str, mode: str = "soft") -> bool:
        ...
        if returncode != 0:
            logger.warning(
                {
                    "event": "vmrun_soft_reset_failed",
                    "vmx": vmx_path,
                    "error": stderr,
                    "message": "Retrying with hard reset...",
                }
            )
            # Fallback to hard reset if soft fails (e.g. VMware Tools not running)
            returncode, stdout, stderr = await self._run_vmrun(
                ["-T", "ws", "reset", vmx_path, "hard"]
            )
            if returncode != 0:
                logger.error(
                    {
                        "event": "vmrun_hard_reset_failed",
                        "vmx": vmx_path,
                        "error": stderr,
                    }
                )
    ...
    async def get_guest_ip(self, vmx_path: str) -> Optional[str]:
        returncode, stdout, stderr = await self._run_vmrun(
            ["-T", "ws", "getGuestIPAddress", vmx_path]
        )
